...
public class CoralPlantFeature extends Feature<ProbabilityFeatureConfiguration> {
    public CoralPlantFeature(Codec<ProbabilityFeatureConfiguration> configCodec) {
        super(configCodec);
    }
    ...
    public boolean place(FeaturePlaceContext<ProbabilityFeatureConfiguration> context) {
        ...
        for(var probabilityZero = 0; probabilityZero < context.config().probability; ++probabilityZero) {
            int random1 = context.random().nextInt(8) - context.random().nextInt(8);
            ...
            BlockState blockstate = JellyfishingBlocks.CORAL_PLANT.get().defaultBlockState();
